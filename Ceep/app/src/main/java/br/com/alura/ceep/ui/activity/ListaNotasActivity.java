...
public class ListaNotasActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        if (ehResultadoInsereNota(requestCode, data)) {
            if(resultadoOk(resultCode)) {
                Nota notaRecebida = getNota(data);
        ...
        if(ehResultadoAlteraNota(requestCode, data)){
            if(resultadoOk(resultCode)) {
                Nota notaRecebida = getNota(data);
                int posicaoRecebida = getPosicao(data);
    ...
    private int getPosicao(Intent data) {
        return data.getIntExtra(CHAVE_POSICAO, POSICAO_INVALIDA);
    }

    private Nota getNota(Intent data) {
        return data.getExtras().getParcelable(CHAVE_NOTA);
    }
    ...
    private boolean ehResultadoInsereNota(int requestCode, @Nullable Intent data) {
        return isCodigoReqInsereNota(requestCode)
                && temNota(data);
    }
    ...
    private boolean isCodigoReqInsereNota(int requestCode) {
